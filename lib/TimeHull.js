...
class TimeHull {
  ...
  getArea(opt) {
    ...
    if (typeof this.area === 'undefined' || !!points) {
      ...
      const perimeterPoints = TimeHull.getDistinctPoints(this.getPolygon({ points }));
  ...
  calculateCentroid(points) {
    const pts = TimeHull.getDistinctPoints(this.getPolygon({ 
      points: [...points] 
    }));
   ...
   * @return {Array}          [description]
   */
  static getDistinctPoints(points) {
    const distinct = [];
