...
class TimeHull {
  ...
  getArea(opt) {
    ...
    if (typeof this.area === 'undefined' || !!opt.points) {
  ...
  getCentroid(opt) {
    ...
    if (typeof this.centroid === 'undefined') {
      ...
      if (points.length === 1) this.centroid = { x: points[0].x, y: points[0].y };
      if (points.length === 2) {
        this.centroid = {
      ...
      if (points.length > 2) {
        const perimeterPoints = TimeHull.distinctPoints(this.polygon({ points }));
        this.centroid = TimeHull.calculateCentroid(perimeterPoints);
      }
    }
  ...
  getCoverage(opt) {
    ...
    if (typeof this.coverage === 'undefined' || !!opt.points) {
      const points = (opt && opt.points) || this.points;
  ...
  distance(which) {
    if (typeof which !== 'undefined') {
      if (this.points && this.points.length > 1) {
        return (
          this.points[this.points.length - 1][which] - this.points[this.points.length - 2][which]
        );
      }
      return 0;
    }
    return Math.sqrt(
      this.distance('x') * this.distance('x') + this.distance('y') * this.distance('y'),
    );
  }
