...
class TimeHullSeries {
  constructor(opt) {
    const { points, period, timestep, includeIncomplete, width, height } = opt || {};
  ...
  getAverageCoverage() {
    if (typeof this.averageCoverage === 'undefined') {
      ...
      if (jStat.sum(this.getHullDurations()) > 0) {
        avg = jStat.sum(this.getCoverageDurations()) / jStat.sum(this.getHullDurations());
      }
  ...
  getFinalCoverage() {
    if (typeof this.finalCoverage === 'undefined') {
      this.finalCoverage = this.getHulls()[this.getHulls().length - 1].getCoverage();
    }
  ...
  getCentroidDistances(opt) {
    ...
    if (typeof this.centroidDistances === 'undefined') {
      ...
      for (let i = 1; i < hulls.length; i += 1) {
        const distX = Math.abs(hulls[i].getCentroid().x - hulls[i - 1].getCentroid().x) || 0;
        const distY = Math.abs(hulls[i].getCentroid().y - hulls[i - 1].getCentroid().y) || 0;
  ...
  getAverageCentroidVelocity(opt) {
    ...
    if (typeof this.averageCentroidVelocity === 'undefined') {
      this.averageCentroidVelocityX =
        jStat.sum(this.getCentroidDistances({ which: 'x' })) / totalHullDuration;
      this.averageCentroidVelocityY =
        jStat.sum(this.getCentroidDistances({ which: 'y' })) / totalHullDuration;
      this.averageCentroidVelocity = jStat.sum(this.getCentroidDistances()) / totalHullDuration;
    }
  ...
  getAverageVelocity(opt) {
    ...
    if (typeof this.averageVelocity === 'undefined') {
      this.averageVelocityX = jStat.sum(this.getDistances({ which: 'x' })) / this.getDuration();
      this.averageVelocityY = jStat.sum(this.getDistances({ which: 'y' })) / this.getDuration();
      this.averageVelocity = jStat.sum(this.getDistances()) / this.getDuration();
    }
   ...
   */
  getEndTime() {
    return this.getHulls()[this.getHulls().length - 1].endTime();
  }
  ...
  getHulls() {
    if (typeof this.hulls === 'undefined') {
      ...
      for (endIndex; startIndex >= 0 && endIndex > 1; endIndex -= 1) {
        ...
        if (typeof startIndex === 'undefined' && (this.includeIncomplete || lastStartIndex > 0)) {
          startIndex = 0;
        }
        ...
        if (startIndex >= 0) {
          ...
          if (
            ...
            this.points[lastEndIndex].timestamp - this.points[endIndex].timestamp >= this.timestep
          ) {
   ...
   */
  getStartTime() {
    return this.getHulls()[0].startTime();
  }
}
