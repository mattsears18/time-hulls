...
describe('TimeHull.coverage()', () => {
  ...
  it('gets the coverage of a custom set of points', () => {
    ...
    const timeHull = new TimeHull({ seriesPoints: dummyPoints, width: 1000, height: 1000 });
  ...
  it('overrides width and height', () => {
    const dummyPoints = [{ x: 1, y: 1, timestamp: 1000 }];
    const timeHull = new TimeHull({ seriesPoints: dummyPoints, width: 10, height: 10 });
    ...
    expect(timeHull.coverage({ points, width: 1000, height: 1000 })).to.equal(0.03);
  });
  ...
  it('has zero stimulus area', () => {
    ...
    const timeHull = new TimeHull({ seriesPoints: points, width: 0, height: 0 });
    expect(() => { timeHull.coverage({}); }).to.throw('noStimulusArea');
  });
});
